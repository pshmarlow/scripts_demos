...
cmd = f'zgrep -hE "{oom_keywords}|{txsentry_keywords}|{reference_data_processor_keywords}" {log_path} 2>/dev/null'
...
def main():
    ...
    seen_events = set()
    process_logs(events_oom, events_txsentry, events_reference_data_processor, seen_events)
    
     for event_list, headers in [(events_oom, headers_oom), (events_txsentry, headers_txsentry), (events_reference_data_processor, headers_reference_data_processor)]:
