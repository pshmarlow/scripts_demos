...
# r'(?P<date>\w{3}\s+\d+\s+\d+:\d+:\d+).*\[(?P<service>\S+)\]...Too many open ...'
...
cmd = f'zgrep -hE "{oom_keywords}|{txsentry_keywords}|{reference_data_processor_keywords}|{expensive_rules_keywords}|{too_many_open_files_keywords}" {log_path} 2>/dev/null'

def process_logs(events_oom, events_txsentry, events_reference_data_processor, events_expensive_rules, events_too_many_open, seen_events):
...
#headers_expensive_rules = [{"key": "time_date", "header": "Time/Date"}]
...
def main():
    ...
    seen_events = set()
    process_logs(events_oom, events_txsentry, events_reference_data_processor, events_expensive_rules, events_too_many_open, seen_events)
    
    for event_list, headers in [(events_oom, headers_oom), (events_txsentry, headers_txsentry), (events_reference_data_processor, headers_reference_data_processor), (events_expensive_rules, headers_expensive_rules), (events_too_many_open, headers_too_many_open)]:
